...
fn main() {
    ...
    for instruction in regex.captures_iter(&data) {
        if instruction.get(0).unwrap().as_str().starts_with("mul") {
            // Mul instruction
            let a = instruction.get(2).unwrap().as_str().parse::<i32>().unwrap();
            let b = instruction.get(3).unwrap().as_str().parse::<i32>().unwrap();
            ...
            bool_mul_enabled = instruction.get(0).unwrap().as_str().starts_with("do(");
        }
    }
